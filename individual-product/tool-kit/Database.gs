...
class Database {
  ...
  /**
   * スプレッドシートからのデータ取得（AND）: ヘッダが特殊な位置から開始する場合
   * @param {String} spreadsheetId
   * @param {String} sheetName
   * @param {Object List} conditions
   ...
   * 
   */
  databaseGetWithSpecificHeader(spreadsheetId, sheetName, headerRow, conditions) {
    try {
      const spreadsheet = this.initSpreadsheet(spreadsheetId); // スプレッドシートを開く
      const sheet = spreadsheet.getSheetByName(sheetName); // 指定したシートを取得
      if (!sheet) throw new Error(this.sheetNameError); // シートを開けなかった場合
      const lastRow = sheet.getLastRow(); // 最後のデータがある行
      const lastColumn = sheet.getLastColumn(); // 最後のデータがある列
      const dataRange = sheet.getRange(headerRow, 1, lastRow - headerRow + 1, lastColumn); // データの範囲を取得
      const values = dataRange.getValues(); // データを2D配列として取得
      const headers = values[0].filter(Boolean); // ヘッダー行を取得（連想配列のキーとして使用）
      ...
      // オフセットと最大取得数の検証
      let offsetRow = 1; // オフセットが未設定または１以下の場合は１に置き換え（カラムは取得しない）
      let limitRow = filteredValues.length; // 取得個数が未設定またはデータ数以上の場合はデータ数に置き換え
  ...
  /**
   * スプレッドシートのデータを更新（AND）: ヘッダが特殊な位置から開始する場合
   * @param {String} spreadsheetId
   * @param {String} sheetName
   ...
   * ];
   */
  databaseUpdateWithSpecificHeader(spreadsheetId, sheetName, headerRow, keyValuePair, filterConditions) {
    try {
      const spreadsheet = this.initSpreadsheet(spreadsheetId); // スプレッドシートを開く
      const sheet = spreadsheet.getSheetByName(sheetName); // 指定したシートを取得
      if (!sheet) throw new Error(this.sheetNameError); // シートを開けなかった場合
      ...
      for (let i = 0; i < filteredValues.length; i++) {
        ...
        if (row[keyIndex] === keyValuePair['key']) {
          for (let updateKey in keyValuePair) {
            ...
            let updateIndex = headers.indexOf(updateKey);
            if (updateIndex !== -1) {
              sheet.getRange(values.indexOf(row) + headerRow, updateIndex + 1).setValue(updateValue);
            }
          }
        }
      }
