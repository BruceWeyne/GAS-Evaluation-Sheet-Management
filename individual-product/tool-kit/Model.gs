...
class Model extends Database {
  ...
  /**
   * データ取得（AND）: ヘッダが特殊な位置から開始する場合
   */
  getDataWithSpecificHeader(sheetName, headerRow, conditions) {
    const dataList = super.databaseGetWithSpecificHeader(this.spreadsheetId, sheetName, headerRow, conditions); // データの取得
    return dataList;
  }
  ...
  /**
   * データ更新（AND）: ヘッダが特殊な位置から開始する場合
   */
  updateDataWithSpecificHeader(sheetName, headerRow, keyValuePairs, conditions) {
    const result = super.databaseUpdateWithSpecificHeader(this.spreadsheetId, sheetName, headerRow, keyValuePairs, conditions); // データの更新
    return result;
  }
